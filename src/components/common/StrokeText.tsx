...
const StrokeText = ({
  ...
  useEffect(() => {
    ...
    const targets = [...strokes, ...fills, strokeEl, fillEl, wipe].filter(Boolean);

    const setStart = () => {
      gsap.killTweensOf(targets);
      gsap.set(strokes, { strokeDasharray: dash, strokeDashoffset: dash });
      if (strokeEl) gsap.set(strokeEl, { strokeDasharray: dash, strokeDashoffset: dash });
      gsap.set(fills, { opacity: useWipe ? 1 : 0 });
      if (fillEl) gsap.set(fillEl, { opacity: useWipe ? 1 : 0 });
    ...
    const setEnd = () => {
      ...
      if (strokeEl) gsap.set(strokeEl, { strokeDasharray: dash, strokeDashoffset: 0 });
      gsap.set(fills, { opacity: fillEnabled ? 1 : 0 });
      if (fillEl) gsap.set(fillEl, { opacity: fillEnabled ? 1 : 0 });
      if (wipe) gsap.set(wipe, { attr: { width: fillEnabled ? box.width + 40 : 0 } });
    };
    ...
    const build = () => {
      ...
      if (strokeEl) {
        tl.to(strokeEl, { strokeDashoffset: 0, duration: drawDuration + (strokes.length - 1) * Number(stagger || 0.05), ease }, 0);
      }

      const strokeTotalTime = (strokes.length - 1) * (typeof stagger === 'number' ? stagger : 0.05) + drawDuration;
      const fillStartTime = Math.max(0.2, strokeTotalTime * 0.55 + fillDelay);

      if (useWipe && wipe) {
        tl.to(
          wipe,
          { attr: { width: box.width + 40 }, duration: fillDuration, ease: 'power2.inOut' },
          fillStartTime
        );
      } else if (fillEnabled) {
        tl.to(
        ...
        if (fillEl) {
          tl.to(fillEl, { opacity: 1, duration: fillDuration, ease: 'power2.out' }, fillStartTime);
        }
      }
